...
public class ReportsServiceIT {
    ...
    public void testAllTicketsReport() {
        ...
        try (MockedStatic<Instant> mocked =
            ...
            TicketAgingReportDTO reports = reportsService.getAgingTicketsReport(queryParams);
            ...
            assertThat(
                            reports.getGroupedTickets().keySet())
                    .containsExactlyInAnyOrder("Alice Johnson", "John Doe", "Jane Smith");
            reports.getGroupedTickets().forEach((key, value) -> assertThat(value.size()).isEqualTo(8));
    ...
    public void testAllOpenTicketsReport() {
        ...
        try (MockedStatic<Instant> mocked =
            ...
            assertThat(
                    reports.getGroupedTickets().keySet())
                    .containsExactlyInAnyOrder("Alice Johnson", "John Doe", "Jane Smith");
            reports.getGroupedTickets().forEach((key, value) -> assertThat(value.size()).isEqualTo(6));
    ...
    public void testAllTicketsFilterByPriority() {
        ...
        try (MockedStatic<Instant> mocked =
            ...
            reports.getGroupedTickets().values().stream().flatMap(List::stream).toList().forEach(item-> assertThat(item.getPriority()).isIn(TicketPriority.High, TicketPriority.Critical));
        }
    }
    ...
    public void testAllTicketsFilterByStatus() {
        ...
        try (MockedStatic<Instant> mocked =
            ...
            assertThat(reports.getGroupedTickets().size()).isEqualTo(2);
            assertThat(reports.getTotalTickets()).isEqualTo(12);
            reports.getGroupedTickets().values().stream().flatMap(List::stream).toList().forEach(item-> assertThat(item.getStatus()).isIn("Backlog", "Ready"));
        }
    }
