...
export default function ProjectBoardView({
  ...
  const handleTaskUpdate = async (updated: TicketDTO) => {
    ...
    try {
      ...
      const stateChanged = old && old.currentStateId !== updated.currentStateId;
      setTasks((prev) => {
        const next = { ...prev };
        if (stateChanged) {
      ...
      >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-7 text-xs gap-1">
                {activeIterationName ?? t.teams.projects.view("all_iterations")}
        ...
        {selectedIteration !== null && (
          <Button
            ...
            onClick={() => {
              const found = iterations.find((i) => i.id === selectedIteration);
              if (found) {
